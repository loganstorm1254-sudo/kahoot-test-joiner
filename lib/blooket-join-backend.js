...
const BLOCKED_MSG =
  "Blooket blocks Vercel server joins (HTTP 403). Set BLOOKET_JOIN_WORKER_URL in Vercel to your Cloudflare join worker URL.";
const USER_AGENT =
...
function getJoinWorkerUrl() {
  return String(process.env.BLOOKET_JOIN_WORKER_URL || "").trim() || null;
}
...
export async function joinBlooketPlayers(gameId, names) {
  ...
  if (blocked) {
    return uniqueNames.map((name) => ({
      name,
      success: false,
      msg: getJoinWorkerUrl() ? BLOCKED_MSG : `${BLOCKED_MSG} Deploy cloudflare/blooket-join-worker.js once with wrangler.`,
      httpStatus: 403,
    }));
  }
