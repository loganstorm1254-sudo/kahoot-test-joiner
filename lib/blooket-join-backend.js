...
async function joinBlooketPlayersViaFetch(gameId, names) {
...
function isBlockedResult(entry) {
  return entry.httpStatus === 403 || /blocked by blooket|cloudflare/i.test(entry.msg || "");
}
...
export async function joinBlooketPlayers(gameId, names) {
  ...
  if (probe.success) {
    ...
    for (const name of uniqueNames.slice(1)) {
      ...
      if (!joinData.success && buildConfig) {
        joinData = await requestJoin(gameId, name, cookies, { encrypted: true, buildConfig });
      }
      results.push({ name, ...joinData });
      await new Promise((resolve) => setTimeout(resolve, 120));
  ...
  if (!isBlockedResult(probe)) {
    const results = [{ name: uniqueNames[0], ...probe }];
    for (const name of uniqueNames.slice(1)) {
      const joinData = await requestJoin(gameId, name, cookies, {
        encrypted: Boolean(buildConfig),
        buildConfig,
      });
      results.push({ name, ...joinData });
  ...
  const { joinBlooketPlayersInBrowser } = await import("./blooket-browser-join.js");
  return joinBlooketPlayersInBrowser(gameId, uniqueNames, buildConfig);
}